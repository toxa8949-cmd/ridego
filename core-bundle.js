...
window.updateHomeStats = function(){ try {
  ...
  // це нижня межа, а не реальна кількість. Сервер (api/home.js) рахує
  // по всій колекції і кладе результат у window.__RIDEGO_STATS__.
  // Беремо більше з двох, щоб цифри не падали після старту SPA.
  var S = window.__RIDEGO_STATS__;
  if (S) {
    nL = Math.max(nL, S.listings || 0);
    nS = Math.max(nS, S.sellers  || 0);
    nC = Math.max(nC, S.cities   || 0);
  }
  if (!nL) return;
...
function _parsePath(path) {
  ...
  if (p === '/elektrosamokat-kyiv-city') return { page: 'catalog', cat: 'Електросамокати' };
  if (p === '/ausom-elektrosamokaty')  return { page: 'catalog', brand: 'ausom' };

  // ausom бракувало в переліку брендів — через це /ausom-l1, /ausom-l2,
  // /ausom-l2-max і /ausom-dt2-pro теж віддавали 404.
  var modelMatch = p.match(/^\/(kukirin|dualtron|xiaomi|ninebot|kaabo|vsett|ausom)-(.+)$/);
...
function _renderRoute(route, isBack) {
  ...
  // Прибираємо серверний контент першого екрана (api/_shell.js).
  // З цього моменту сторінку малює SPA, тож два однакові блоки
