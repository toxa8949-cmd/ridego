...
function renderChats() { if(window._renderChats) window._renderChats(); }
function openChatById(id) { if(window._openChatById) window._openChatById(id); }
function _renderMessages(m,c) { if(window.__renderMessages) window.__renderMessages(m,c); }
function sendMessage() { if(window._sendMessage) window._sendMessage(); }
function _startChatFromListing() { if(window.__startChatFromListing) window.__startChatFromListing(); }
function openChat() { showPage("messages"); }
function closeChatOnMobile() { var l=document.querySelector(".messages-layout"); if(l) l.classList.remove("chat-open"); window._activeChatId=null; renderChats(); }
function _startChat(u,l,t) { if(window.__startChat) window.__startChat(u,l,t); }
function _formatChatTime(s) { if(!s) return ""; var d=new Date(s*1000),n=new Date(); return (n-d<86400000&&d.getDate()===n.getDate())?d.toLocaleTimeString("uk-UA",{hour:"2-digit",minute:"2-digit"}):d.toLocaleDateString("uk-UA",{day:"numeric",month:"short"}); }
