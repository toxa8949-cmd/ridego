...
document.addEventListener('visibilitychange', function() {
  if (document.hidden) {
    if (_chatsUnsubscribe) {
      ...
      _chatsUnsubscribe = null;
      window.__chatListenersCount = Math.max(0, window.__chatListenersCount - 1);
      console.log('[RideGO] Chats unsubscribed (tab hidden) | Active listeners:', window.__chatListenersCount);
    }
  } else {
    // Вкладка знову активна — відновлюємо підписку якщо залогінені
    if (currentUser && currentUser.uid && window._db) {
      _chatsSubscribedUid = null; // скидаємо щоб дозволити нову підписку
      _subscribeChats();
    }
  }
});
